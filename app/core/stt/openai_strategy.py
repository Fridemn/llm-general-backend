...
class ProviderOpenAISTT(STTProvider):
    ...
    def _process_sync(self, audio_file: Union[str, Path, BinaryIO], return_full_response: bool = False, **kwargs) -> Union[str, Dict[str, Any]]:
        """
        处理音频文件（同步方式）
    ...
    async def transcribe(self, audio_file: str, **kwargs) -> str:
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None,
                lambda: self._process_sync(audio_file, **kwargs)
            )
