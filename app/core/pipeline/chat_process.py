...
class ChatProcess:
    ...
    ) -> Union[StreamingResponse, LLMResponse, Dict[str, Any]]:
        """
        处理统一聊天请求
    ...
    async def _prepare_input_message(
        ...
        try:
            ...
            if stt and audio_file:
                ...
                transcribed_text = await voice_process.process_stt(audio_file_path)
                ...
                text_component = MessageComponent(type=MessageType.TEXT, content=transcribed_text)
                ...
                # 构造输入消息
                return LLMMessage.from_text(text=message, history_id=history_id or "", role=role)
            ...
            try:
                ...
                if tts and full_response_text.strip() and message_id and current_history_id:
                    ...
                    if audio_url:
                        # 创建音频组件
                        audio_component = voice_process.create_audio_component(
                            audio_url, full_response_text, {"tts_model": "default"}
                        )
    ...
    async def _handle_normal_response(
        self, model: str, input_message: LLMMessage, history_id: Optional[str], user_id: Optional[str], tts: bool
        ...
        try:
            ...
            if tts and response.response_message.message_str.strip():
                ...
                if audio_url:
                    # 创建音频组件并添加到响应
                    audio_component = voice_process.create_audio_component(
                        audio_url, response.response_message.message_str, {"tts_model": "default"}
                    )
