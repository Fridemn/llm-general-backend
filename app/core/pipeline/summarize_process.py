    ...
    async def generate_history_title(self, history_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            input_message = LLMMessage.from_text(
                text=prompt, history_id="", role=MessageRole.USER  # 空字符串，让API自动创建新历史记录
            )
            ...
            response = await text_process.process_message(summary_model, input_message)
            ...
            return {"success": True, "history_id": history_id, "new_title": title}
